...
export default function Todo({onEdit, data}) {
  const ref = useRef();
  const [state, send] = useService(data.ref);
  const { id, text, completed } = state.context;

  useEffect(() => {if(ref.current != undefined) ref.current.checked = completed})
  ...
  return (
    <li className="group w-full leading-tight rounded-none border-b border-gray-250 relative">
        ...
        <input ref={ref} type="checkbox" className="absolute h-full opacity-0 w-43px label-checked:bg-check label-checked:line-through label-checked:text-gray-450 appearance-none" defaultChecked={completed} onChange={e => {
          console.log(e)
          send("TODO.EDIT", {id, text, completed: e.target.checked})
        }}/>
